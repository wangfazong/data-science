...
path = '/Users/wangfazong/dataScience/kaggle/'

train_data = pd.read_csv(path+'train.csv')
test_data = pd.read_csv(path+'test.csv')

print('Total number of question pairs for trainning:{}'.format(len(train_data)))
print('Duplicate pairs: {}%'.format(round(train_data['is_duplicate'].mean()*100, 2)))
qids = pd.Series(train_data['qid1'].tolist() + train_data['qid2'].tolist())
print('Total number of question:{}'.format(len(np.unique(qids))))
print('Number of question appear multiple:{}'.format(np.sum(qids.value_counts() > 1)))

# plt.figure(figsize=(12,5))
# plt.hist(qids.value_counts(), bins=50)
# plt.yscale('log', nonposy='clip')
# plt.title("log-his appear counts")
# plt.xlabel('number of occur of question')
...
# dist_train2 = train_qs.apply(lambda x:len(x.split(' ')))
# dist_test2 = test_qs.apply(lambda x:len(x.split(' ')))
# plt.figure(figsize=(15, 10))
